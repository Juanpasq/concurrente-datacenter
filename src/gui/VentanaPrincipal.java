...
public class VentanaPrincipal extends JFrame {
    ...
    public VentanaPrincipal(VentanaEstadisticas vStats) {
        ...
        scrollLog.setPreferredSize(new Dimension(500, 130)); // Fijamos un tamaño para que no compita con los stats
        
        // Panel de Estadísticas (A la derecha del log)
        JPanel pnlStats = new JPanel(new GridLayout(6, 1, 5, 5)); // 6 filas, 1 columna, 5px de espacio
    ...
    private JLabel crearLabelEquipo(String nombre, Color color) {
        ...
        label.setPreferredSize(new Dimension(105, 45));
                ...
                while (true) {
                    ...
                    try {
                       ...
                       //AIRES ACC y boton stress aires
                        if (vPri.temperatura >= 30.0) {
                            if (!vPri.emergenciaTermica) {
                                ...
                                vPri.escribirLog("🚨 EMERGENCIA TÉRMICA: Sistemas apagados por seguridad (>30°C)");

                            }
                        } else if (vPri.temperatura < 25.0) {
                            if (vPri.emergenciaTermica) {
                                vPri.emergenciaTermica = false;
                                vPri.escribirLog("✅ AMBIENTE SEGURO: Sistemas autorizados a reanudar.");
                            }
                        }
                     ...
                     // Dentro del try del hilo de refresco en el main
                        ...
                        // CALOR: Sumamos si los servidores están prendidos (NO están en rojo)
                        if (!vPri.lblProd.getBackground().equals(Color.RED)) cambioTemp += 0.06;
                        if (!vPri.lblNas.getBackground().equals(Color.RED))  cambioTemp += 0.06;
                        if (!vPri.lblLab.getBackground().equals(Color.RED))  cambioTemp += 0.06;

                        // FRÍO: Restamos si los aires están enfriando (Celeste)
                        Color colorAireActivo = new Color(173, 216, 230);
                        if (vPri.lblAire1.getBackground().equals(colorAireActivo)) cambioTemp -= 0.12;
                        if (vPri.lblAire2.getBackground().equals(colorAireActivo)) cambioTemp -= 0.12;

                        // Actualizamos la variable de la ventana
                        vPri.temperatura += cambioTemp;

                        // Límites de seguridad para que no sea infinito
                        if (vPri.temperatura < 16) vPri.temperatura = 16; // Los aires no congelan más que eso
                        if (vPri.temperatura > 45) vPri.temperatura = 45; // Punto de falla crítica
